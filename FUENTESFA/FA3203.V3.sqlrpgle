    ...
    *  Para Copiar el Presupuesto de clientes Migrado
     ...
     c+       Select Cli As CliCve,
     c+              Art As ArtCve,
     c+              Round(Sum(Can),-0) As DtoCuo,
     c+              Round((Sum(Can)/:Meses),-0) As CuaPro,
     c+              Round(Max(Can),-0) As CuoMax,
     c+              Round(Avg(Can),-0) As CuoAvg
     ...
     c                   EndIf
        ...
        //Crear Cuota de Ventas por Clientes y Productos
           Exec Sql
              Merge Into FacCpc as T
              Using ( Values(:CliCve, :ArtCve, :CuoPro, :CuoMax, :DtoCua) )
                    as S(Cli, Art, Pro, Max, Tot)
                 On T.CliCve = S.Cli
                And T.ArtCve = S.Art

              -- Solo Inserta Cuando No existe
              When Not Matched Then
                   Insert (CliCve, ArtCve, CpcCvc, CpcCvm, CpcCvt,
                           CpcCdd, CpcCvp)
                   Values (S.Cli, S.Art, S.Pro, S.Max, S.Tot,
                           0, 0);
