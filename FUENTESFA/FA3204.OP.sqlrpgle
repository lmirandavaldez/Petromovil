      ...
      *  IDENTIFICACION:                                                 *
      *  ---------------                                                 *
      *  NOMBRE DEL PROGRAMA .........: FA3204_OPT                       *
      ...
      *       un INSERT INTO ... SELECT masivo  mucho mas eficiente     *
      ...
      *
      **FA3204_OPT Prototype
     d FA3204_OPT      Pr
     d  CodSup                        2
     d  CodVen                        3
     d  FechaP                        8
      *
      **FA3204_OPT Program Interface
     d FA3204_OPT      Pi
     d  CodSup                        2
     d  CodVen                        3
     d  FechaP                        8
      ...
      *
      /Free
        ...
        // Proceso Creacion Tabla Principal (INSERT masivo)
           Exsr Impresion;
        ...
        // Proceso Para Borrar Tabla Temporal
           ...
           Exsr EndProgram;
        // ------------------------------------------------------
        // Proceso para la carga y calculo de metricas          -
        // ------------------------------------------------------
           Begsr Proceso       ;

        //Insertar clientes Activos solo Compras por Pedidos
           Exec Sql
        ...
        //Actualizar Ultima Compra
           ...
           If SqlCod < *Zeros ;
             SqlMsgTxt = 'Proceso: UPDATE FechaUlt' ;
             Exsr ErrSql ;
           Endif ;

        //Calcular Promedio de Dias entre Compras (usando LAG)
           Exec Sql
           ...
           If SqlCod < *Zeros ;
             SqlMsgTxt = 'Proceso: INSERT Promdias' ;
             Exsr ErrSql ;
           Endif ;

        //Actualizar Dias Promedio de Compras en Churnwk
           Exec Sql
             Update Qtemp/Churnwk T1
                Set PromDias = (
                    Select P.PromDias
           ...
           If SqlCod < *Zeros ;
             SqlMsgTxt = 'Proceso: UPDATE PromDias' ;
             Exsr ErrSql ;
           Endif ;
        ...
        //Actualizar Monto y Cantidad del Periodo Anterior (12-24 meses)
           ...
           EndSr ;
        // ------------------------------------------------------
        // MEJORA: INSERT masivo desde SELECT  sin loop cursor  -
        ...
        --Ranking por vendedor basado en % churn
           ...
           EndSr ;
        // ------------------------------------------------------
        // Crear Tablas Temporales en QTEMP                     -
        // ------------------------------------------------------
           Begsr Crear_Tabla   ;
        ...
        //Tabla temporal dias promedio de compras
           ...
           If SqlCod < *Zeros ;
             SqlMsgTxt = 'Crear_Tabla: CREATE TABLE Promdias' ;
             Exsr ErrSql ;
           Endif ;

           EndSr ;
        // ------------------------------------------------------
        // MEJORA: Borrar Tablas solo si existen en QTEMP       -
        // Evita SQLCODE -204 si el proceso aborto antes         -
        // de crearlas.                                          -
        // ------------------------------------------------------
           Begsr Borrar_Tabla  ;
           ...
           // SQLCODE 0 = borrada, +100 = no existia: ambos son OK
           If SqlCod < *Zeros ;
           ...
           Exec Sql
           Drop Table If Exists Qtemp/Promdias ;

           If SqlCod < *Zeros ;
             SqlMsgTxt = 'Borrar_Tabla: DROP Promdias' ;
             Exsr ErrSql ;
           Endif ;

           EndSr ;
        // -----------------------------------------------------
        // MEJORA: Subrutina de error SQL                      -
        // Registra SQLCODE y SQLSTATE del error y termina     -
        // el programa de forma controlada.                    -
        // -----------------------------------------------------
           Begsr ErrSql        ;
          ...
          SqlMsgTxt = %TrimR(SqlMsgTxt) + ' | SQLCODE=' +
                      %Trim(%Char(SqlErrCod)) + ' SQLSTATE=' + ErrSqlState;
