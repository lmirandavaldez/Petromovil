              ...
              Set Monto12 = (
                Select Sum(T1.DtoImp)
                 ...
                 Where (T1.CliCve = T.CliCve)
                   And (T9.FecIso >= current_date - 12 months)
                   And (T3.ArtPpr = 'S')
                   And (T4.DisTip = 'N'));
        ...
        //Actualizar Monto de Compras de los Ultimos 24 Meses
           ...
           Select Sum(T1.DtoImp)
             ...
             Join SegFec T9
              ...
              And (T1.DtoDia = T9.FecDia)
            Where (T1.CliCve = T.CliCve)
              And (T9.FecIso Between (current_date - 24 months)
                   And (current_date - 12 months))
              And (T3.ArtPpr = 'S')
              And (T4.DisTip = 'N'));
        ...
        // ------------------------------------------------------
           ...
           Exec Sql
            ...
            Select T4.SupCve As Codigo_Del_Supervisor,
                   ...
                   Coalesce(days(current_date) - days(T1.FechaUlt), 99999)
                   As Dias_Sin_Comprar,
                   ...
                   Case When T1.FechaUlt Is Null
                             ...
                             Or PromDias = 0
                             Then 'SIN HISTORIAL'
                   When Days(Current_Date) - Days(T1.FechaUlt) <= PromDias
                             Then 'ACTIVO'
                   When Days(Current_date) - Days(T1.FechaUlt) <= PromDias * 1.5
                             Then 'EN RIESGO'
