      ...
      *
     d tx              s             50    dim(04) ctdata perrcd(1)
      *
     d parcve          s              1    Inz('@')
...
L004 d CodigoDis       s                   Like(DisCve) Inz(*Zeros)
     ...
     d NcdFdaIso       s               d   Datfmt(*Iso)
     d NcdFdaAmd       s                   Like(FecDre)
      *
L001  * Parametros
...
L001  /Copy Fuentes,SG9001
 ...
 ''   *
     ...
     c                   Select
     c                   When      TipDoc = 2
     c                   Eval      Wtxt = %Trim(Tx(1))
      *
     c                   When      TipDoc = 8
     c                   Eval      Wtxt = %Trim(Tx(3))
     c                   EndSl
      *
     c                   Eval      *In44 = *On
     c                   Eval      FecDre = *Date
     ...
     c                   Select
     c                   When      TipDoc = 2
     c                   Eval      Wtxt = %Trim(Tx(2))
      *
     c                   When      TipDoc = 8
     c                   Eval      Wtxt = %Trim(Tx(4))
     c                   EndSl
      *
     c                   Exsr      chenea
     ...
     c                   Leave
     c                   EndIf
      * Almacen no permite venta
     c                   If        AlmVen = 'N'
     c                   Eval      *In30 = *On
     c                   Eval      *In70 = *On
     c                   Eval      msgid = 'INV0045'
     c                   Exsr      error_snd
     c                   Leave
     c                   EndIf
      *
 ...
 ''  c                   Exsr      Busca_Vendedor
     ...
     c                   Eval      NcdFdaAmd = %Dec(%Date(FecDre:*Eur):*Iso)
      *
     c                   Clear                   Existe            1 0
     c                   If        NcdDre > *Zeros
      * Determina si existen registros relacionados
     c/Exec Sql
     ...
     c                   EndIf
       ...
       // Verificar que las Identificaciones sean iguales
        SqlFacDed = *Blanks     ;
           Exec Sql
              Select *
                Into :SqlFacDed
       ...
       // No debe Existir Nota de Credito o Debito Pendiente del Mismo Documento
        Status = *Off     ;
           Exec Sql
              Select '1'
                Into :Status
     ...
     c                   When      TipDoc = 2
     c                   Eval      FolCve = 004
     c                   Eval      FolDes = 'Numero Secuencia Nota Credito'
      * Nota de Debito
     c                   When      TipDoc = 8
     c                   Eval      FolCve = 003
     c                   Eval      FolDes = 'Numero Secuencia Nota Debito'
     c                   EndSl
      *
...
L001 c                   EndSr
      ...
      * ----------------------------------------------------------
**
  Alta a Cabecera Nota de Credito por Devolución
 Cambio a Cabecera Nota de Credito por Devolución
   Alta a Cabecera Nota de Debito por Devolución
  Cambio a Cabecera Nota de Debito por Devolución
